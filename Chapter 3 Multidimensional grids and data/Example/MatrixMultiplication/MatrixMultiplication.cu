...
// P = M dot N
__global__
void MatrixMultiplicationKernel(int *M, int *N, int *P, const uint MatrixSize)
{
    uint row_index {blockDim.y * blockIdx.y + threadIdx.y};
    uint column_index {blockDim.x * blockIdx.x + threadIdx.x};
    if (row_index < MatrixSize && column_index < MatrixSize)
    {
        int value {0};
...
void MatrixMultiplication(int *M_h, int *N_h, int *P_h, const size_t MatrixSize)
{
    ...
    MatrixMultiplicationKernel<<<Griddim, Blockdim>>>(M_d, N_d, P_d, MatrixSize);
